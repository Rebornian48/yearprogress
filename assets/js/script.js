...
function countdown() {
  ...
  var tahunBaru =
    hariDepan + ", " + tanggalDepan + " " + bulanDepan + " " + tahunDepan;
  ...
  var pass =
    Math.round(current_date.getTime() - currNewYear.getTime()) / one_day_ms;
  var rest =
    Math.round(newYear.getTime() - current_date.getTime()) / one_day_ms;
  var PassResult = pass.toFixed(8);
  var RestResult = rest.toFixed(8);

  document.getElementById("passed").innerText = PassResult;
  document.getElementById("remaining").innerText = RestResult;

  var passPercentage = ((pass / (pass + rest)) * 100).toFixed(8);
